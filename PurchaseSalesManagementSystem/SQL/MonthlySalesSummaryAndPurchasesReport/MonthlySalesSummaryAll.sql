WITH #ProjItems AS (SELECT ItemCode FROM CI_Item WHERE ItemType = 1)
, BASEDATA AS ( 
    SELECT
        *
...
, KData AS ( 
    ...
    FROM
        ...
        LEFT JOIN ( 
            ...
            ON SOQ.ItemCode = #ProjItems.ItemCode 
        LEFT JOIN ( 
            SELECT
                ItemCode
                , SUM( 
            ...
            ON RAI.ItemCode = #ProjItems.ItemCode
) 
, ARData AS ( 
    SELECT
    ...
    FROM
        ...
        LEFT JOIN #ProjItems 
            ON #ProjItems.ItemCode = IM_ItemTransactionHistory.ItemCode 
    WHERE
...
WHERE
    COALESCE(Qty1, 0) <> 0 
    OR COALESCE(Qty2, 0) <> 0 
    OR COALESCE(Qty3, 0) <> 0 
    OR COALESCE(Qty4, 0) <> 0 
    OR COALESCE(Qty5, 0) <> 0 
    OR COALESCE(Qty6, 0) <> 0 
    OR COALESCE(Qty7, 0) <> 0 
    OR COALESCE(Qty8, 0) <> 0 
    OR COALESCE(Qty9, 0) <> 0 
    OR COALESCE(Qty10, 0) <> 0 
    OR COALESCE(Qty11, 0) <> 0 
    OR COALESCE(Qty12, 0) <> 0 
    OR COALESCE(InvData.[OnHand(EOM)], 0) <> 0 
    OR COALESCE(KData.TotalQuantityOnHand, 0) <> 0 
    OR COALESCE(KData.TotalSalesOrder, 0) <> 0 
    OR COALESCE(KData.TotalPurchaseOrder, 0) <> 0 
ORDER BY
    KData.ItemCode
;